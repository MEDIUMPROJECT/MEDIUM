...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #MEDIUMTEST2\r");
            Send(hSocket, "WHO #MEDIUMTEST2\r");
        } else {
            // randomly join #MEDIUM00-#MEDIUM05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #MEDIUM%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #MEDIUM%02d\r", channel_number).c_str());
        }
